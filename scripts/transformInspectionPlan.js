...
function execute() {
  ...
  if (existingPdf && existingPdf.isDocument) {
    ensureVersionable(existingPdf);
    existingPdf.properties["content"].write(transformedPdf.properties["content"]);
    existingPdf.save();
    logger.error("[transformInspectionPlan] Updated existing PDF content: " + inspectionFolder.displayPath + "/" + targetName);
  } else {
    transformedPdf.copy(inspectionFolder);
    logger.error("[transformInspectionPlan] Copied new PDF to folder: " + inspectionFolder.displayPath + "/" + targetName);
  }
