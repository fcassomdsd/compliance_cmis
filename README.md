...
| `POST /api/inspection/import-canonical` | Import canonical checklist/finding/follow-up data into the inspection folder | `inspectionCode`, `specialtyName` (see also the follow-up importer below) | `startDate`, `endDate`, `locale`, context ids. The canonical base path is server-resolved — `sourceBasePath` in the payload is ignored. | `example/import-canonical-checklist.sample.json`, `example/import-canonical-finding.sample.json` |
